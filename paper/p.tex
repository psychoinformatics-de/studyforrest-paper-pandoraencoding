...
Encoding models learn linear filters that predict recorded activity in each voxel
from a stimulus representation, thus providing information how well features of the stimulus can explain brain activity.
...
Yet, researchers have many degrees of freedom in constructing voxel-wise encoding models, such as the type of stimulus
representation, the number of voxels to use, or which voxels to include in the analysis.

%ohne multiple
Accordingly, to evaluate the influence of these choices, multiple quality metrics
exist that assess how well encoding models approximate brain activity across multiple
voxels.
...
stimuli, these studies leave large gaps in our understanding how analysis
parameters and BOLD acquisition affect these different validation strategies.
In this study, we aim to narrow these gaps, by documenting the effect of field
strength, voxel selection strategies, and numbers of voxels used on different
quality metrics of voxel-wise encoding models.

To this end, we utilize a 3T f{MRI} study on the perception of musical genres
\citep{CTK+2012} which has recently been replicated using a 7T fMRI scanner \citep{HDH+2015}.
The main goal of this study is thus to apply three approaches to encoding validation, binary retrieval
