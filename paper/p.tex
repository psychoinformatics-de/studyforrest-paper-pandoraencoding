...
\citep{TD+06,KG+08,NG11,huth2012continuous,holdgraf2016rapid,holdgraf2017encoding}.
...
While this regularization parameter can exert a large influence on regression coefficients, and hence interpretation, we do not scrutinize its role here, since we are interested in choosing the amount of regularization that maximizes prediction accuracy.
...
correct matches for all (exhaustive) pair-wise combinations of stimuli, independent of genre,
and then dividing by the number of combinations. This was then averaged across stimuli.
...
observed BOLD activity of every stimulus, $y_{n}$ for $n=1..25$. These
...
$r2$: 250 voxels, 6.75 $cm^{3}$ volume, 7T and $r2$: 500 voxels, 0.68 $cm^{3}$ volume, 3T and stability selection: 500
...
$r2$: 1000 voxels, 27 $cm^{3}$ volume, 7T and $r2$: 5000 voxels, 13.5 $cm^{3}$ volume, 3T and stability selection: 500
...
A consistent advantage of 3T versus 7T is visible in decoding individual stimuli: if voxels are selected by stability, 3T outperforms 7T for
...
for selection by $r2$ and selection by stability, 6.75 $cm^{3}$ volume), and subsequently decreases if
...
Our results show that all three quality metrics varying with the number of
...
Our results thus show the significance of on an often under-appreciated aspect of
...
voxel-wise encoding models.
Incidentally, one of our results -- a higher matching score in 7T than in 3T fMRI data -- constrasts a  previous study \citep{SF14}, which found higher matching scores for encoding natural sounds in 7T than in 3T, although using a different number of stimuli in the two conditions.
%TODO: rewrite
This difference is not surprising given the large degrees of freedom in each study, whose substantial effects on all quality metrics are clearly visible.

However, in spite of these inconsistencies, some common patterns emerge that allow us to abstract several recommendations for applying and validating
voxel-wise encoding models.
...
While we believe that these three recommendations can guide neuroimaging researchers somewhat, to conclusively assess if these recommendations generalize, more data are
