...
musical genre. We do this by varying either field strength, comparing
performance in a 3-Tesla and a 7-Tesla dataset, and parameters of the data
analysis, specifically, the number of voxels used and how they were selected.
While not including stimulus reconstruction as in \cite{NG09}, we use the same
method of constructing a probabilistic decoding model from a set of encoding
models, and use it to infer the music genre of the presented stimulus.
...
p(y_{vt}|x_{t}) = N(y_{vt};x_{t}\beta_{v},\sigma) \end{equation} where $N(y;\mu,\sigma)$ denotes the probability density at point $y$ for a gaussian with mean $\mu$ and standard deviation $\sigma$, and $\beta_{v}$ is a $[Mx1]$ vector of regression coefficients specific to voxel $v$.  \subsection*{Quality metrics} \todo[inline]{add description of standardization}
