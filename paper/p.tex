...
%
Binary retrieval accuracy \citep[as described in][]{ML08} estimates if
...
of stimuli and then averaged.  In a prior study \citep{CTK+2012}, using the
...
\todo[inline]{change N}
%
An alternative measure of encoding performance is the correlation rank score
\citep{SF14}. The predicted f{MRI} activity $\widetilde{i}_{n}$ is computed as
in \ref{eq:predict} \todo{undefined ref} for each stimulus $s_{n}$ $n=1,2..N$
in the validation set.  For each stimulus, the correlations between
$\widetilde{i}_{n}$ and the measured f{MRI} activity of all stimuli is
computed, and the matching score $m(s_{n})$ is then
...
of a decoder built from all individual encoding models \citep{NG11}. To go
from $p(y_{vt}|x_{t})$ to $p(x_{t}|y_{t})$ we follow \citet{NG09} and first
...
measured f{MRI} activity onto them. Like \citet{NG09} we construct the
...
over music stimuli $p(s_{i}|y)$ (for $i=1..25$ (??\todo{??})). We use the simplifying
...
$p(l_{i}|y) \propto p(x_{t-1}|y_{t-1})p(x_{t}|y_{t})p(x_{t+1}|y_{t+1})$\todo{worth its own line IMHO} where
